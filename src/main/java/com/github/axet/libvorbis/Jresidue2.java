...
final class Jresidue2 extends Jresidue1 {
	...
	/** designed for stereo or other modes where the partition size is an
	   ...
	   submap */
	private static int[][] _2class(final Jvorbis_block vb, final Object vl, final int[][] in, final int ch) {
...
/* if( TRAIN_RESAUX ) {
	...
	final int[][] fclass(final Jvorbis_block vb, final Jvorbis_look_residue vl,
	...
	final int forward(final Joggpack_buffer opb,
				final Jvorbis_block vb, final Jvorbis_look_residue vl,
...
//#endif
	...
	final int inverse(final Jvorbis_block vb, final Jvorbis_look_residue vl,
				...
				for( int l = 0; i < partvals; l++ ) {
					...
					for( int k = 0; k < partitions_per_word && i < partvals; k++, i += samples_per_partition ) {
						...
						if( (secondstages[ p_k ] & s1) != 0 ) {
							...
							if( stagebook != null ) {
								if( stagebook.vorbis_book_decodevv_add( in,
										i, ch,
