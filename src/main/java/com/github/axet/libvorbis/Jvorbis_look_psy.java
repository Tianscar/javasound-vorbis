...
final class Jvorbis_look_psy implements Icomparator {
	...
	final void _vp_psy_init(final Jvorbis_info_psy vinfo,
					final Jvorbis_info_psy_global gi, final int size, final int rate) {
		...
		this.firstoc = (int)(Jcodec.toOC( .25f * rate * .5f / size ) * (1 << (this.shiftoc + 1)))
				- gi.eighth_octave_lines;
		final int maxoc = (int)(Jcodec.toOC((size + .25f) * rate * .5f / size) * (1 << (this.shiftoc + 1)) + .5f);
...
/*if( false ) {// #if 0
	...
	final void _vp_couple_quantize_normalize(final int blobno,
