...
*/	//
	...
	private static void vorbis_lsp_to_curve(final float[] curve, final int[] map, final int n, final int ln,
...
//eop:
	...
	private static void floor0_map_lazy_init(final Jvorbis_block vb, final Object infoX, final Jvorbis_look_floor0 look) {
		if( look.linearmap[vb.W] == null ) {
			...
			for( j = 0; j < n; j++ ) {
				int val = (int)Math.floor( Jcodec.toBARK( v * (float)j ) * scale ); /* bark numbers represent band edges */
				if( val >= look.ln ) {
	...
	final boolean inverse2(final Jvorbis_block vb, final Jvorbis_look_floor i, final Object memo, final float[] out) {
		final Jvorbis_look_floor0 look = (Jvorbis_look_floor0)i;
		final Jvorbis_info_floor0 info = look.vi;
