...
*/
	...
	public final int vorbis_block_init(final Jvorbis_block vb) {
		...
		if( this.analysisp != 0 ) {
			final Jvorbis_block_internal vbi =
				vb.m_internal = new Jvorbis_block_internal();
			vbi.ampmax = -9999;
			...
			if( ci.fullbooks == null ) {
				...
				for( int i = 0; i < books; i++ ) {
					...
					if( book_param[i] == null ||
						ci.fullbooks[i].vorbis_book_init_decode( book_param[i] ) != 0 )
					{// abort_books:
						for( i = 0; i < books; i++ ) {
							if( book_param[i] != null ) {
	...
	private static float vorbis_lpc_from_data(final float[] data, final int offset,
...
//done:
	...
	private static void vorbis_lpc_predict(final float[] coeff,
...
/* #if 0
				...
				vorbis_lpc_predict( lpc, work, delta - order,
		...
		if( vals <= 0 ) {
			...
			for( int i = 0, channels = vinfo.channels; i < channels; i++ ) {
				...
				if( eof_flag > (order << 1) ) {
					...
					/* run the predictor filter */
					vorbis_lpc_predict( lpc, v_pcm_i, eof_flag - order, order,
...
/* #if 0
	...
	private final int _ve_envelope_search() {
		...
		final float[][] pcm =  this.pcm_data.pcm;// java
		for( int j = first, js = searchstep * j, jp = j + Jenvelope_lookup.VE_POST; j < last; j++, js += searchstep ) {
			int ret = 0;
