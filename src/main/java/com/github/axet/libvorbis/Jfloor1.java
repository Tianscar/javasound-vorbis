...
*/
	...
	private static int accumulate_fit(final float[] flr,
					...
					final int x0, int x1, final Jlsfit_acc a,
					final int n, final Jvorbis_info_floor1 info)
	{
		int i;
	...
	private static boolean fit_line(final Jlsfit_acc[] a, final int offset, int fits,
					final int[] y0_y1, final Jvorbis_info_floor1 info)
	{
		int y0 = y0_y1[0];
		int y1 = y0_y1[1];
	...
	static int[] floor1_fit(final Jvorbis_block vb, final Jvorbis_look_floor1 look,
		...
		final int[] fit_valueA = new int[Jvorbis_info_floor.VIF_POSIT + 2]; /* index by range list position */
		final int[] fit_valueB = new int[Jvorbis_info_floor.VIF_POSIT + 2]; /* index by range list position */

		final int[] loneighbor = new int[Jvorbis_info_floor.VIF_POSIT + 2]; /* sorted index of range list position (+2) */
	...
	static int[] floor1_interpolate_fit(final Jvorbis_block vb, final Jvorbis_look_floor1 look,
	...
	static boolean floor1_encode(final Joggpack_buffer opb, final Jvorbis_block vb,
		...
		if( post != null ) {
			...
			for( int i = 0, j = 2, ie = info.partitions; i < ie; i++ ) {
				...
				if( csubbits != 0 ) {
					...
					/* write it */
					look.phrasebits +=
							books[ info.class_book[iclass] ].vorbis_book_encode( cval, opb );
...
//eop:
	...
	@Override
	final boolean inverse2(final Jvorbis_block vb, final Jvorbis_look_floor in, final Object memo, final float[] out) {
